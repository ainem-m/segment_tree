var segTree = new Vue({
  ...
  data: {
    ...
    identity: {'sum': 0, 'mul': 1, 'max': Number.NEGATIVE_INFINITY, 'min': Number.POSITIVE_INFINITY},
    n: 4,
  ...
  methods:{
    ...
    func:function(a, b){
      ...
      // セグ木に載せる機能
      // this.typeOfSegmentTreeによって変える
      switch(this.typeOfSegmentTree){
        case 'sum':
          return a+b
        case 'mul':
          return a*b
    ...
    init:function(){
      ...
      this.resetSelected()
      values.fill(Number(this.identity[this.typeOfSegmentTree]))
      for (i=0; i<size; i++) {
        // 最下段に元の値を代入　欠損あれば単位元で埋める
        if (i<n) values[i+size] = args[i] ? args[i] : this.identity[this.typeOfSegmentTree];
        names[i+size] = [(i+1).toString()]
      }
      for (i=size-1; i>0; i--){
        // 下から二段目から上へ計算
        values[i] = this.func(values[i*2], values[i*2+1]);
    ...
    query:function(){
      loop = (l, r, tmp) => {
        ...
        if (l & 1) {
          ...
          // |..|**|
          // 左端・右の子(処理に含める)
          tmp = this.func(tmp, this.values[l++]);
        } else if (l !== r-1){
          // |->...|
          // |->|..|
          // 左端・左の子(処理に含めない)
        ...
        if (r & 1) {
          ...
          // |**|<-|
          // 右端・右の子(処理に含める)
          tmp = this.func(tmp, this.values[--r]);
        } else if (l !== r) {
          // |<-...|
          // |<-|..|
          // 右端・左の股(処理に含めない)
      ...
      var tmp = this.identity[this.typeOfSegmentTree];
    ...
    update:function(){
      ...
      loop = (i) => {
        ...
        this.values.splice();
        this.values[i] = this.func(this.values[2*i], this.values[2*i + 1]);
